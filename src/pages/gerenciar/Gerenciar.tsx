...
import OvinoImg from "../../assets/images/ovinoImagem.png"
...
useEffect(() => {
  ...
  const filteredOvinos = useMemo(() => {
    ...
    return ovinos.filter((o) =>
      [o.rfid.toString(), o.fbb, o.nome, o.raca, o.sexo]
        .filter(Boolean)
        .some((field) =>
  ...
  const filteredFuncionarios = useMemo(() => {
    ...
    return funcionarios.filter((c) =>
      [c.cpfCnpj.toString(), c.cpfCnpj, c.endereco, c.nome, c.telefone]
        .filter(Boolean)
        .some((field) =>
          field!.toLowerCase().includes(searchQuery.toLowerCase())
        )
